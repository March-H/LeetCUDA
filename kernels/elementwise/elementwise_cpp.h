#ifndef ELEMENTWISE_CPP_H
#define ELEMENTWISE_CPP_H
...
inline __device__ void elementwise_add_f32_kernel(float *a, float *b, float *c,
                                                  int N) {
...
inline __device__ void elementwise_add_f32x4_kernel(float *a, float *b, float *c,
                                                    int N) {
...
// ElementWise Add grid(N/256),
// block(256) a: Nx1, b: Nx1, c: Nx1, c = elementwise_add(a, b)
inline __device__ void elementwise_add_f16_kernel(half *a, half *b, half *c, int N) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N)
...
// a: Nx1, b: Nx1, c: Nx1, c = elementwise_add(a, b)
inline __device__ void elementwise_add_f16x2_kernel(half *a, half *b, half *c, int N) {
...
inline __device__ void elementwise_add_f16x8_kernel(half *a, half *b, half *c, int N) {
...
inline __device__ void elementwise_add_f16x8_pack_kernel(half *a, half *b, half *c,
                                                         int N) {
    int idx = 8 * (blockIdx.x * blockDim.x + threadIdx.x);
